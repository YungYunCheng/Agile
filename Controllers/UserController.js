        ...
        User.register(new User(newUser), req.body.password, 
                function(err, account) {
                    ...
                    passport.authenticate('local') (req, res, 
                            function () { res.redirect('/User/MyReview'); });
                });
...
exports.LoginUser = (req, res, next) => {
  ...
  passport.authenticate('local', {
      successRedirect : '/Flash Card/MyFlashCard', 
...
// This displays a view called 'MyReview' but only 
// if user is authenticated.
exports.MyFlashCard  = async function(req, res) {
    let reqInfo = RequestService.reqHelper(req);
    let flashcardperuser = _flashcardRepo.getFlashCardPerUser(reqInfo.username)

    if(reqInfo.authenticated) {
        res.render('Flash Card/MyFlashCard', {errorMessage:"", reqInfo:reqInfo, flashcard: flashcardperuser})
    }
    else {
...
// This displays a view called 'profile' but only 
// if user is authenticated.
exports.Profile  = async function(req, res) {
    let reqInfo = RequestService.reqHelper(req);

    if(reqInfo.authenticated) {
        res.render('User/Profile', {errorMessage:"", reqInfo:reqInfo, user: reqInfo.username})
    }
    else {
        res.redirect('/User/Login?errorMessage=You ' + 
                     'must be logged in to view this page.')
